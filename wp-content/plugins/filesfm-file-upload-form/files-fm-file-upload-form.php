...
define('FILES_FM_UPLOAD__VERSION', '1.3.1');
...
define('FILES_FM_UPLOAD__EMBED_SCRIPT_HOST', '//files.fm');
...
function files_fm_upload__shortcode_handler( $atts ) {
    ...
    {
        $str_embed_code = '<script type="text/javascript" id="files_fm_upload_script_tag" src="' . FILES_FM_UPLOAD__EMBED_SCRIPT_HOST . FILES_FM_UPLOAD__EMBED_SCRIPT_PATH . '?uid='.$str_user_id.'&uid_hash='.$str_user_hash.'&insert_before_id=files_fm_upload_script_tag'. $config .''. $lang .'"></script>';
    }
    elseif ( ! empty( $str_username ) && ! empty( $str_addhash ) )
    {
        $str_embed_code = '<script type="text/javascript" id="files_fm_upload_script_tag" src="' . FILES_FM_UPLOAD__EMBED_SCRIPT_HOST . FILES_FM_UPLOAD__EMBED_SCRIPT_PATH . '?addto='.$str_username.'&addhash='.$str_addhash.'&insert_before_id=files_fm_upload_script_tag'. $config .''. $lang .'"></script>';
    }
	elseif ( ! empty( $str_iframe_src ) )
    {
        $str_embed_code = '<iframe src="'. $str_iframe_src . $config . $lang .'" frameborder="0" style="border: 0; width: '. (empty($str_iframe_width) ? '500px': $str_iframe_width) .'; height:'. (empty($str_iframe_height) ? '500px': $str_iframe_height) .'; overflow-x: hidden; max-width: none !important;"></iframe>';
    }
    
    return $str_embed_code;
}
...
function files_fm_upload__uninstall () {
    ...
    $str_url = FILES_FM_UPLOAD__ADMIN_ORIGIN . FILES_FM_UPLOAD__ADMIN_PATH . '?uninstall=1&lng=' . $str_lang_code . $str_user_parameters;
